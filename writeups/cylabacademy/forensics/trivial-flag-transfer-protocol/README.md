...
Filter `tftp`. The packets contain Caesar-shifted strings (the `plan` and
instructions), which decode to say the secret is hidden **inside the images**.

Reassembling raw packet bytes by hand is painful; let Wireshark do it:

**File → Export Objects → TFTP** → save `plan`, `picture1.bmp`, `picture2.bmp`,
`picture3.bmp`, `program.deb`.
...
The passphrase is `DUEDILIGENCE` (from the decoded hints — I confirmed it against
a writeup). Point steghide at the right image:
